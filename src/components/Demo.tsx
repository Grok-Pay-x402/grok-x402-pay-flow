...
export const Demo = ({ isWalletConnected }: { isWalletConnected: boolean }) => {
  ...
  return (
    <section className="py-20 px-4 border-t border-border" id="demo">
      <div className="container mx-auto max-w-3xl">
        ...
        <div className="border border-border p-8">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <Textarea
                placeholder={
                  useCase === "chat" 
                    ? "explain_defi_in_simple_terms"
                    : useCase === "image"
                    ? "cyberpunk_city_at_sunset"
                    : "monitor_eth_price_alert_on_5%_change"
                }
                value={prompt}
                onChange={(e) => setPrompt(e.target.value)}
                className="min-h-32 font-mono bg-secondary border-border resize-none"
                disabled={isLoading}
              />
            </div>
          ...
          {txHash && (
            <div className="mt-6 p-4 border border-border bg-secondary">
          ...
          {result && (
            <div className="mt-6 p-6 border border-border bg-secondary">
                ...
                onClick={() => {
                  window.open(
                    `https://twitter.com/intent/tweet?text=${encodeURIComponent(
                      "Just tested GrokPay x402. Pay 0.01 USDC for AI with zero subscriptions. #x402 #GrokPay #Web3AI"
                    )}`,
                    '_blank'
                  );
                }}
        ...
        <div className="mt-12 text-center">
          <div className="grid grid-cols-3 gap-4 text-sm font-mono">
            <div>
              ...
              <div className="text-xs text-muted-foreground">TESTS_TODAY</div>
            </div>
            <div>
